...
//         private void LoadAndParseSourceFiles(string path)
//         {
//             Directory.GetFiles(path, "*.*", SearchOption.AllDirectories)
//                 .Where(f => f.EndsWith(".json") && !f.StartsWith("conf"))
//                 .Select(LoadFile<Dictionary<string, object>>);// TODO parse

//             // .ToDictionary(
//             //     f => Path.Combine(Path.GetDirectoryName(f[(path.Length + 1)..]) ?? "", Path.GetFileName(f)),
//             //     f =>
//             //     {
//             //         var o = LoadFile<Dictionary<string, object>>(f);
//             //         // o.Name = Path.GetFileNameWithoutExtension(f);
//             //         return o;
//             //     }
//             // );
...
//             var sourceFolder = VerefyFolderPath(rootFolder, "Schema");
...
//             //TESTING ONLY:
//             var schema = dbSO.FindProperty($"<{nameof(LocalizationDatabase.Schema)}>k__BackingField");
//             var jsonTexts = Directory.GetFiles(sourceFolder, "*.*", SearchOption.AllDirectories)
//                 .Where(f => f.EndsWith(".json") && !f.StartsWith("conf"))
//                 .Select(File.ReadAllText);

//             schema.boxedValue = LocalizationSchemaParser.TryParse(jsonTexts, out var result) ? result : default;
