...
namespace Lostbyte.Toolkit.CustomEditor
{
        public class RequiredAttribute : CombinedAttribute
        {
#if UNITY_EDITOR
                private bool _error;
                public delegate bool ValidatorDelegate(SerializedProperty property);
                [ClearStatic] private static readonly Dictionary<string, ValidatorDelegate> _validators = new();
                public static void Register(string propertyType, ValidatorDelegate validator)
                {
                        _validators[propertyType] = validator;
                }
                private static bool TryValidate(SerializedProperty property, out bool isEmpty)
                {
                        if (_validators.TryGetValue(property.type, out var validator))
                        {
                                isEmpty = validator(property);
                                return true;
                        }
                ...
                {
                        if (TryValidate(property, out bool customIsEmpty))
                        {
