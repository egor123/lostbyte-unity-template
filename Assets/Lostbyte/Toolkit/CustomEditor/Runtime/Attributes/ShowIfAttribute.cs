        ...
        private bool ShouldShow(SerializedProperty property)
        {
            ...
            if (TargetNames != null && TargetNames.Length > 0)
            {
                foreach (var target in TargetNames)
                {
                    ...
                    {
                        Print.Warn($"ShowIf: Cannot find '{target}' on {type.Name} {property.isArray}");
                        continue;
                    }
                    if (value switch
                    {
            ...
            else if (TargetTypes != null && TargetTypes.Length > 0)
            {
                ...
                {
                    Print.Warn($"ShowIf: Target must be Component for type check!");
                    return false;
                }
                return TargetTypes.Any(t => IncludeChildrenTypes ? c.GetComponentInChildren(t) : c.GetComponent(t));
            }
            Print.Warn($"ShowIf: No valid condition");
            return false;
        }
